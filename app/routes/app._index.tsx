// ─────────────────────────────────────────────────────────────────────────────
// app._index.tsx — Smart hero: "Upload first catalog" vs "View My Catalogs"
// Loader checks if merchant has any catalogs and passes hasCatalogs flag
// ─────────────────────────────────────────────────────────────────────────────
...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    const Q = `query GetPDFCount { shop { metafields(first: 1, namespace: "PDF") { edges { node { id } } } } }`;
    const res = await admin.graphql(Q);
    const data = await res.json();
    const count = data?.data?.shop?.metafields?.edges?.length ?? 0;
    return json({ hasCatalogs: count > 0 });
  } catch {
    return json({ hasCatalogs: false });
  }
};

const CatalogIllustration = () => (
  <svg viewBox="0 0 480 320" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ width: "100%", maxWidth: 480, height: "auto" }}>
...
const steps = [
  {
    number: "01", title: "Upload your catalog",
    description: "Drag and drop any PDF — product lookbooks, seasonal catalogs, brochures. We handle the rest.",
    color: "#EFF6FF", accent: "#1A73E8",
    icon: <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><path d="M12 15V4M12 4l-4 4M12 4l4 4" stroke="#1A73E8" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" /><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1" stroke="#1A73E8" strokeWidth="1.75" strokeLinecap="round" /></svg>,
  },
  {
    number: "02", title: "Tag your products",
    ...
    icon: <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3" stroke="#22C55E" strokeWidth="1.75" /><circle cx="12" cy="12" r="8" stroke="#22C55E" strokeWidth="1.75" strokeDasharray="3 3" /></svg>,
  },
  {
    number: "03", title: "Publish to your store",
    description: "Copy your catalog key, paste it into the theme block, and go live in seconds.",
    color: "#FFF7ED", accent: "#F59E0B",
    icon: <svg width="20" height="20" fill="none" viewBox="0 0 24 24"><path d="M5 12l5 5L19 7" stroke="#F59E0B" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" /></svg>,
  },
];

const features = [
  { title: "Page-flip experience", description: "Your catalog opens like a real book — smooth page turns that keep shoppers engaged.", accent: "#1A73E8", bg: "#EFF6FF", border: "#BFDBFE" },
  { title: "Hotspot product pins", description: "Clickable pins sit directly on products. Customers see pricing and add to cart without leaving the page.", accent: "#22C55E", bg: "#F0FDF4", border: "#BBF7D0" },
  { title: "Fully customisable", description: "Control hotspot colors, button styles, tooltip design, and animation type from global settings.", accent: "#F59E0B", bg: "#FFFBEB", border: "#FDE68A" },
];

const quickActions = [
  {
    label: "Upload new catalog", desc: "Add another PDF to your catalog library.",
    accent: "#1A73E8", bg: "#EFF6FF", border: "#BFDBFE", path: "/app/pdf-convert",
    icon: <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path d="M12 15V4M12 4l-4 4M12 4l4 4" stroke="#1A73E8" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" /><path d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1" stroke="#1A73E8" strokeWidth="1.75" strokeLinecap="round" /></svg>,
  },
  {
    label: "Global settings", desc: "Adjust hotspot styles, animations, and button design.",
    accent: "#F59E0B", bg: "#FFFBEB", border: "#FDE68A", path: "/app/global-settings",
    icon: <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><path d="M12 15a3 3 0 100-6 3 3 0 000 6z" stroke="#F59E0B" strokeWidth="1.75" /><path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-4 0v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 010-4h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 012.83-2.83l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 014 0v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 2.83l-.06.06A1.65 1.65 0 0019.4 9a1.65 1.65 0 001.51 1H21a2 2 0 010 4h-.09a1.65 1.65 0 00-1.51 1z" stroke="#F59E0B" strokeWidth="1.75" /></svg>,
  },
  {
    label: "Setup guide", desc: "Learn how to embed your catalog in the storefront.",
    accent: "#22C55E", bg: "#F0FDF4", border: "#BBF7D0", path: "/app/installation",
    icon: <svg width="18" height="18" fill="none" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="#22C55E" strokeWidth="1.75" /><path d="M12 8v4M12 16h.01" stroke="#22C55E" strokeWidth="1.75" strokeLinecap="round" /></svg>,
  },
];

const Index = () => {
  const { hasCatalogs } = useLoaderData<typeof loader>();
  ...
  return (
    <div style={{ minHeight: "100vh", backgroundColor: "#F8FAFC", fontFamily: "'DM Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif" }}>
      <style>{`
        ...
        .actionCard:hover { transform: translateY(-1px); box-shadow: 0 6px 20px rgba(15,23,42,0.08) !important; }
      `}</style>
            ...
            {hasCatalogs ? (
              <>
                <h1 style={{ fontSize: 36, fontWeight: 700, color: "#0F172A", lineHeight: 1.2, margin: "0 0 14px", letterSpacing: "-0.03em" }}>
                  Welcome back —{" "}
                  <span style={{ color: "#1A73E8" }}>your catalogs are live</span>
                </h1>
                <p style={{ fontSize: 15, color: "#64748B", lineHeight: 1.7, maxWidth: 440, margin: "0 0 30px" }}>
                  Manage your shoppable catalogs, update hotspot pins, or upload a new PDF to keep your store fresh.
                </p>
                <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                  <button className="heroBtn" onClick={() => navigate("/app/pdf-convert")}
                    ...
                    <svg width="15" height="15" fill="none" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1.5" stroke="white" strokeWidth="2" /><rect x="14" y="3" width="7" height="7" rx="1.5" stroke="white" strokeWidth="2" /><rect x="3" y="14" width="7" height="7" rx="1.5" stroke="white" strokeWidth="2" /><rect x="14" y="14" width="7" height="7" rx="1.5" stroke="white" strokeWidth="2" /></svg>
                    View my catalogs
                  </button>
                  <button className="ghostBtn" onClick={() => navigate("/app/global-settings")}
      ...
      {/* ── HOW IT WORKS — new merchants only ── */}
      {!hasCatalogs && (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
            {steps.map((step) => (
              <div key={step.number} className="stepCard"
                ...
                <div style={{ width: 40, height: 40, borderRadius: 10, background: step.color, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14 }}>
                  {step.icon}
                </div>
      ...
      {/* ── QUICK ACTIONS — returning merchants only ── */}
      {hasCatalogs && (
        <div style={{ padding: "52px 48px 0", maxWidth: 1008, margin: "0 auto" }}>
          <div style={{ marginBottom: 28 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: "#1A73E8", letterSpacing: "0.1em", textTransform: "uppercase" }}>Quick actions</span>
            <h2 style={{ fontSize: 22, fontWeight: 700, color: "#0F172A", margin: "6px 0 0", letterSpacing: "-0.02em" }}>What would you like to do?</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
            {quickActions.map((action) => (
              <button key={action.label} className="actionCard" onClick={() => navigate(action.path)}
                style={{ background: action.bg, border: `1px solid ${action.border}`, borderRadius: 12, padding: "22px 20px", textAlign: "left", cursor: "pointer", transition: "transform 0.15s, box-shadow 0.15s" }}>
                <div style={{ width: 38, height: 38, borderRadius: 9, background: "#fff", border: `1px solid ${action.border}`, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 12 }}>
                  {action.icon}
                </div>
                <h3 style={{ fontSize: 14, fontWeight: 600, color: "#0F172A", margin: "0 0 5px" }}>{action.label}</h3>
                <p style={{ fontSize: 13, color: "#64748B", lineHeight: 1.6, margin: 0 }}>{action.desc}</p>
              </button>
            ))}
          </div>
        </div>
      ...
      <div style={{ padding: "44px 48px 0", maxWidth: 1008, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 14 }}>
          {features.map((f) => (
            <div key={f.title} style={{ background: f.bg, border: `1px solid ${f.border}`, borderRadius: 12, padding: "22px 20px" }}>
      ...
      <div style={{ padding: "44px 48px 60px", maxWidth: 1008, margin: "0 auto" }}>
        <div style={{ background: "#fff", border: "1px solid #E8EDF2", borderRadius: 14, padding: "32px 36px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, flexWrap: "wrap" }}>
          <div>
            <h3 style={{ fontSize: 18, fontWeight: 700, color: "#0F172A", margin: "0 0 4px", letterSpacing: "-0.02em" }}>
              {hasCatalogs ? "Add another catalog" : "Ready to get started?"}
            </h3>
            <p style={{ fontSize: 13, color: "#64748B", margin: 0 }}>
              {hasCatalogs
                ? "Keep your store fresh with a new lookbook, seasonal collection, or brochure."
                : "Upload your first catalog and see it live in your store within minutes."}
            </p>
          </div>
          <button className="heroBtn" onClick={() => navigate("/app/pdf-convert")}
            ...
            {hasCatalogs ? "Upload new catalog" : "Upload your first catalog"}
